...
class ImageRequest {
  ...
  async getOriginalImage(bucket, key) {
    ...
    try {
      ...
      if (originalImage.ContentType) {
        // if (
        //   originalImage.ContentType === "image/gif" ||
        //   originalImage.ContentType === "gif"
        // ) {
        //   // If animated GIF, skip.
        //   const imageBuffer = Buffer.from(originalImage.Body);
        //   this.checkAnimatedGif(imageBuffer);
   ...
   * @param {Buffer} imageBuffer - Image buffer.
   */
  checkAnimatedGif(imageBuffer) {
    if (Animated(imageBuffer)) {
      throw {
        status: 500,
        code: "GifNotSupportedError",
        message: "This service does not support animated gif.",
      };
    }
  }
}
